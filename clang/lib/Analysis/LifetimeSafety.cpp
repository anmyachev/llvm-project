...
private:
  ...
  /// \param Path The expression representing the path to a object being
  /// borrowed.
  /// \param IssuedTo The expression that triggers the borrow (e.g., an
  /// address-of unary operator). The loan is issued to the origin of this
  /// expression.
  /// \return The new Loan on success, nullptr otherwise.
  const Loan *createLoanFrom(const Expr *Path, const Expr *IssuedTo) {
    ...
    // etc.
    if (const auto *DRE = dyn_cast<DeclRefExpr>(Path->IgnoreParenImpCasts())) {
      if (const auto *VD = dyn_cast<ValueDecl>(DRE->getDecl())) {
        AccessPath Path(VD);
        return &FactMgr.getLoanMgr().addLoan(Path, IssuedTo);
      }
    }
    return nullptr;
  }
